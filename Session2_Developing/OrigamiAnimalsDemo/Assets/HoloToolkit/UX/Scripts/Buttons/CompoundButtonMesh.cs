...
namespace HoloToolkit.Unity.Buttons
{
    ...
    public class CompoundButtonMesh : ProfileButtonBase<ButtonMeshProfile>
    {
            ...
            if (!Profile.SmoothStateChanges)
            {
                ...
                if (Renderer != null)
                {
                    if (instantiatedMaterial == null)
                    {
                        ...
                        instantiatedMaterial = Instantiate(sharedMaterial) as Material;
                        instantiatedMaterial.CopyPropertiesFromMaterial(sharedMaterial);
                        instantiatedMaterial.mainTexture = Instantiate(sharedMaterial.mainTexture) as Texture2D;
                    ...
                    if (!string.IsNullOrEmpty(Profile.ColorPropertyName))
                    {
                        ...
                        UpdateMaterial(Renderer.sharedMaterial);
                    }
        ...
        {
            mat.EnableKeyword("_USEMAINCOLOR_ON");
            mat.EnableKeyword("_SPECULARHIGHLIGHTS_ON");
            mat.EnableKeyword("_USEMAINTEX_ON");
            mat.EnableKeyword("_USEAMBIENT_ON");
            mat.EnableKeyword("_USEDIFFUSE_ON");
        }
